...
<table border="1">
<tr><th>Seat Id</th><th>Seat Name</th><% if("india".equalsIgnoreCase(user)){%><th>Action</th><% }%>
...
try
{
	...
	ResultSet rs=st.executeQuery("select * from dv_seat");
	while(rs.next())
	{
		
		%>
	<tr><td><%=rs.getInt(1)%></td><td><%=rs.getString(2)%></td><% if("india".equalsIgnoreCase(user)){%><td><a href="edit_seats.jsp?seatid=<%=rs.getInt(1)%>&seatname=<%=rs.getString(2)%>">edit</a>|<a href="delete_seat.jsp?seatname=<%=rs.getString(2)%>">delete</a></td><% }%></tr>		
<%
	}
	con.close();
